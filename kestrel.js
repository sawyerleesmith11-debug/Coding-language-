...
function parse(tokens) {
  ...
  function parseType() {
    if (at("[")) {
      ...
      expect(";");
      const size = expect("IDENT").value; // N as a symbolic bound, or a number lexed as IDENT
      expect("]");
  ...
  function isImpure(fn, stack = new Set()) {
    ...
    function visitStmt(s) {
      ...
      switch (s.kind) {
        ...
        case "assign":
          if (!locals.has(s.name)) { impure = true; return; } // mutating something outside its own locals
          visitExpr(s.value);
          break;
